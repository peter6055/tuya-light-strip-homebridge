    ...
    getSendParam(name, value) {
        ...
        switch (name) {
            ...
            case Characteristic.Brightness:
                ...
                //percentage = Math.floor((100 - 0) * value / 100 + 0); //  value 0~100
                code = "colour_data"

                var saturation = this.getCachedState(Characteristic.Saturation) / 100 * 1000;
                //saturation = Math.floor((1000 - 0) * this.getCachedState(Characteristic.Saturation) / 100 + 0); // value 0~100
                var hue = this.getCachedState(Characteristic.Hue);
                ; // 0-359
            ...
            case Characteristic.Hue:
                var bright = this.getCachedState(Characteristic.Brightness) / 100 * 1000;
                var saturation = this.getCachedState(Characteristic.Saturation) / 100 * 1000;

                //bright = Math.floor((1000 - 0) * this.getCachedState(Characteristic.Brightness) / 100 + 0); //  value 0~100
                //saturation = Math.floor((1000 - 0) * this.getCachedState(Characteristic.Saturation) / 100 + 0); // value 0~100
                code = "colour_data";
                value = {
                    "h": value,
            ...
            case Characteristic.Brightness:
                return {
                    "commands": [
                        {
                            "code": "colour_data",
                            "value": ColorProtocol.encodeColorData(value.h, 1000, value.v)
                        },
                        {
                            "code": "video_mode",
